...
*   **Sensitivity** or **True Positive Rate (TPR)** measures the proportion of positives that are correctly identified as such; it is the same as recall; it is calculated as TP/(TP+FN);
*   **Specificity** or **True Negative Rate (TNR)** measures the proportion of negatives that are correctly identified as such; it is calculated as TN/(TN+FP).
...
We can extract the **Area Under the Curve (AUC)** value from the output of the *roc* function:
...
A rather high value (recall that AUC takes values in the [0,1] range, and the higher it is, the better).
...
Note that the accuracy and sensitivity (recall) are the same as in thecorresponding column (column 5 that corresponds to the chosen threshold) in the nb2.coords matrix.
